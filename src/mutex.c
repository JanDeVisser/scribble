...
#endif /* HAVE_PTHREAD_H */
    trace("Trylock mutex: %s", (retval) ? "Fail" : "Success");
    return retval;
}
...
{
    trace("Trying to acquire condition");
...
void condition_sleep(Condition condition)
{
    ...
    trace("Going to sleep on condition");
#ifdef HAVE_PTHREAD_H
