...
ErrorOrChar read_file_by_name(char const *file_name)
{
    ...
    if (fd < 0) {
        ERROR(Char, IOError, "Could not open file");
    }
...
ErrorOrChar read_file(int fd)
{
    struct stat sb;
    if (fstat(fd, &sb) < 0) {
        ERROR(Char, IOError, "Could not fstat file");
    }
    size_t sz = sb.st_size;
    char* buffer = mem_allocate(sz + 1);
    if (!buffer) {
        ERROR(Char, OutOfMemory, "Out-of-memory allocating file buffer");
    }
    buffer[sz] = 0;
    size_t read_chars = read(fd, buffer, sz);
    if (read_chars < sz) {
        ERROR(Char, IOError, "Could not read file");
    }
    RETURN(Char, buffer);
}
